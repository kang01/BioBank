...
public interface StockOutFrozenBoxRepository extends JpaRepository<StockOutFrozenBox,Long> {
    ...
    @Query("SELECT S FROM StockOutFrozenBox S WHERE S.stockOutTask.stockOutPlan.stockOutApply.applyCode = ?2 AND (S.frozenBoxCode1D IN ?1 OR S.frozenBoxCode IN ?1)")
