...
public interface TranshipBoxRepository extends JpaRepository<TranshipBox,Long> {
    ...
    @Query(value = "select t.* from tranship_box t where t.tranship_id =?1 and t.status!='0000' and t.status!='2090'" ,nativeQuery = true)
    List<TranshipBox> findByTranshipId(Long id);

    @Query(value = "select t.* from tranship_box t where t.frozen_box_code =?1 and t.status!='0000' and t.status!='2090'",nativeQuery = true)
    TranshipBox findByFrozenBoxCode(String frozenBoxCode);

    @Modifying
    @Query("update TranshipBox t set t.status=?1 where t.tranship.id=?2 and t.status not in ('2090','0000')")
