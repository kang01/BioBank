package org.fwoxford.service.mapper;

import org.fwoxford.domain.*;
import org.fwoxford.service.dto.TranshipBoxPositionDTO;

import org.mapstruct.*;
import java.util.List;

/**
 * Mapper for the entity TranshipBoxPosition and its DTO TranshipBoxPositionDTO.
 */
@Mapper(componentModel = "spring", uses = {})
public interface TranshipBoxPositionMapper {

    @Mapping(source = "equipment.id", target = "equipmentId")
    @Mapping(source = "area.id", target = "areaId")
    @Mapping(source = "supportRack.id", target = "supportRackId")
    @Mapping(source = "transhipBox.id", target = "transhipBoxId")
    TranshipBoxPositionDTO transhipBoxPositionToTranshipBoxPositionDTO(TranshipBoxPosition transhipBoxPosition);

    List<TranshipBoxPositionDTO> transhipBoxPositionsToTranshipBoxPositionDTOs(List<TranshipBoxPosition> transhipBoxPositions);

    @Mapping(source = "equipmentId", target = "equipment")
    @Mapping(source = "areaId", target = "area")
    @Mapping(source = "supportRackId", target = "supportRack")
    @Mapping(source = "transhipBoxId", target = "transhipBox")
    TranshipBoxPosition transhipBoxPositionDTOToTranshipBoxPosition(TranshipBoxPositionDTO transhipBoxPositionDTO);

    List<TranshipBoxPosition> transhipBoxPositionDTOsToTranshipBoxPositions(List<TranshipBoxPositionDTO> transhipBoxPositionDTOs);

    default Equipment equipmentFromId(Long id) {
        if (id == null) {
            return null;
        }
        Equipment equipment = new Equipment();
        equipment.setId(id);
        return equipment;
    }

    default Area areaFromId(Long id) {
        if (id == null) {
            return null;
        }
        Area area = new Area();
        area.setId(id);
        return area;
    }

    default SupportRack supportRackFromId(Long id) {
        if (id == null) {
            return null;
        }
        SupportRack supportRack = new SupportRack();
        supportRack.setId(id);
        return supportRack;
    }

    default TranshipBox transhipBoxFromId(Long id) {
        if (id == null) {
            return null;
        }
        TranshipBox transhipBox = new TranshipBox();
        transhipBox.setId(id);
        return transhipBox;
    }
}
